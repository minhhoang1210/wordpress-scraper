...
export async function renderCover(title: string, author: string): Promise<Uint8Array | null> {
  const width = 1200
  const height = 1800
  ...
  const lines = wrapText(ctx, title.toUpperCase(), width - 260, 'bold 76px Georgia, serif')
